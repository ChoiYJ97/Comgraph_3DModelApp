	...
	D3DXMATRIX baseViewMatrix)
{
	...
	result = UpdateSentence(m_sentence3, "Vertex: ", 20, 60, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetVerCount(int vertexCount, ID3D11DeviceContext *deviceContext)
{
	char tempString[20];
	char VertexString[20];
	bool result;

	// Convert the cpu integer to string format.
	_itoa_s(vertexCount, tempString, 10);

	// Setup the cpu string.
	strcpy_s(VertexString, "Vertex: ");
	strcat_s(VertexString, tempString);
	strcat_s(VertexString, "#");

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, VertexString, 20, 60, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
